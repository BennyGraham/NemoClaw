...
| `NEMOCLAW_NON_INTERACTIVE_SUDO_MODE` | `prompt` | Allows non-interactive onboarding to use prompt-capable `sudo` for host setup steps that require elevation. By default, non-interactive onboarding uses `sudo -n` and fails instead of asking for a password. |
