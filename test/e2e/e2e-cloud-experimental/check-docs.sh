...
run_install_check() {
  ...
  local COMMANDS_MD="$REPO_ROOT/docs/reference/commands.md"
  if [[ ! -f "$COMMANDS_MD" ]]; then
    echo "check-docs: [install] missing $COMMANDS_MD" >&2
    return 1
  fi

  local _doc_provider_row _doc_provider_values
  _doc_provider_row="$(grep -E '^\| `NEMOCLAW_PROVIDER` \|' "$COMMANDS_MD" || true)"
  if [[ -z "$_doc_provider_row" ]]; then
    echo "check-docs: [install] no NEMOCLAW_PROVIDER row found in ${COMMANDS_MD#"$REPO_ROOT"/}" >&2
    _drift=1
  else
    ...
    while IFS= read -r v; do
      ...
      if ! grep -qxF -- "$v" <<<"$_doc_provider_values"; then
        echo "check-docs: [install] provider \"$v\" canonical but absent from ${COMMANDS_MD#"$REPO_ROOT"/} NEMOCLAW_PROVIDER row" >&2
        _drift=1
      fi
    ...
    while IFS= read -r v; do
      [[ -z "$v" ]] && continue
      if ! grep -qxF -- "$v" <<<"$_canonical_values"; then
        echo "check-docs: [install] provider \"$v\" appears in ${COMMANDS_MD#"$REPO_ROOT"/} NEMOCLAW_PROVIDER row but is not canonical" >&2
        _drift=1
      fi
