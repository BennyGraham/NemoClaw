  ...
  while (args.length > 0) {
    ...
    if (arg === "--root") opts.root = path.resolve(args.shift() ?? "");
      ...
      process.stdout.write(
        "tsx scripts/e2e/render-parity-report.ts [--root <repo>] [--parity-json <file>] [--coverage-report <file>] [--output <file>]\n",
      );
      process.exit(0);
    } else {
...
function scenarioRows(scenarios: ScenariosYaml) {
  ...
  for (const [id, scenario] of Object.entries(scenarios.setup_scenarios ?? {})) {
    ...
    const fullOnboardBlocked =
      platformId === "macos-local"
...
function main() {
  ...
  const scenarios = readYaml<ScenariosYaml>(
    path.join(opts.root, "test/e2e/nemoclaw_scenarios/scenarios.yaml"),
  );
  ...
  const coverage =
    opts.coverageReport && fs.existsSync(opts.coverageReport)
  ...
  lines.push("");
  lines.push(
    "This report summarizes legacy E2E assertion conversion, scenario coverage, and current parity comparison output. It is intended to make coverage gaps visible while the scenario runner is being restructured into base environment scenarios, onboarding overlays, and post-onboard feature suites.",
  );
  lines.push("");
  lines.push(
    mdTable(
    ...
    lines.push(
      mdTable(
        ...
        [
          ...
          [
            "Assertions not yet converted in this comparison",
            String(liveParity.counts?.deferred ?? 0),
          ],
  ...
  lines.push(
    mdTable(
      ...
      scenarioRows(scenarios).map((row) => [
        row.id,
        row.base,
        row.install,
        row.runtime,
  ...
  lines.push("");
  lines.push(
    "Platform gap to call out: the macOS scenario is currently not expected to complete full Docker-backed onboarding on hosted macOS because Docker is not available there. Other non-negative scenarios are intended to run full onboarding when their runner and secret requirements are satisfied.",
  );
  lines.push("");
  ...
  lines.push(
    mdTable(
      ...
      topUnconverted.map((row) => [
        row.script,
        String(row.mapped),
        String(row.notConverted),
        String(row.retired),
      ]),
    ),
  );
  ...
  lines.push("");
  lines.push(
    "The scenario framework increases visibility by separating setup dimensions, expected-state contracts, and post-onboard suites. The next coverage improvement is to classify unconverted assertions by destination: base environment setup, onboarding flow, expected-state validation, post-onboard feature suite, negative/failure mode, or retire candidate.",
  );
  lines.push("");
  lines.push(
    "Priority areas suggested by the current parity map are onboarding lifecycle, messaging providers, security/shields, sandbox lifecycle, GPU/Ollama, credential sanitization, and inference routing.",
  );
  ...
  if (coverage) {
    ...
    lines.push(
      coverage
        ...
        .replace(
          /\| Bucket \| Scripts \| Mapped \| Deferred \| Retired \| Unmapped \|/g,
          "| Bucket | Scripts | Mapped | Assertions not yet converted | Retired | Unmapped |",
        ),
    );
  }
